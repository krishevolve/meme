        ...
        for (const quest of pendingQuests) {
          ...
          if (quest.status === "CLAIM") {
            ...
            if (!result.success || result.data.status !== "VERIFY") {
              this.log(`Không thể bắt đầu nhiệm vụ ${quest.id} | ${quest.title}: ${result.error || "Invalid status"}`, "error");
              continue;
            }
            ...
            if (!result.success || result.data.status !== "CLAIM") {
              this.log(`Không thể xác minh nhiệm vụ ${quest.id} | ${quest.title}: ${result.error || "Invalid status"}`, "error");
              continue;
            }
            ...
            if (!result.success || result.data.status !== "DONE") {
              this.log(`Không thể hoàn thành nhiệm vụ ${quest.id} |  ${quest.title}: ${result.error || "Invalid status"}`, "error");
              continue;
            }
          }
